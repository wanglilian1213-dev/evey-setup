...
var
  ...
  ProviderUrlPage: TInputQueryWizardPage;
...
begin
  ...
  SecretPage := CreateInputQueryPage(PluginPage.ID,
    'AI provider keys',
    'Model provider setup is optional.',
    'You can deploy Hermes first and configure models or account login later. Values are hidden and written to .env. They are not passed to PowerShell command-line arguments.');
  SecretPage.Add('OpenAI API key (optional):', True);
  SecretPage.Add('Kimi / Moonshot API key (optional):', True);
  SecretPage.Add('Qwen / DashScope API key (optional):', True);
  SecretPage.Add('GLM / Z.AI API key (optional):', True);
  SecretPage.Add('OpenRouter API key (optional):', True);
  SecretPage.Add('Telegram bot token (optional):', True);
  SecretPage.Add('Discord bot token (optional):', True);

  ProviderUrlPage := CreateInputQueryPage(SecretPage.ID,
    'AI provider URLs',
    'Optional API URLs for model providers.',
    'Leave these defaults unless your provider, region, or proxy gives you a different URL.');
  ProviderUrlPage.Add('OpenAI API URL:', False);
  ProviderUrlPage.Values[0] := 'https://api.openai.com/v1';
  ProviderUrlPage.Add('Kimi / Moonshot API URL:', False);
  ProviderUrlPage.Values[1] := 'https://api.moonshot.ai/v1';
  ProviderUrlPage.Add('Qwen / DashScope API URL:', False);
  ProviderUrlPage.Values[2] := 'https://dashscope.aliyuncs.com/compatible-mode/v1';
  ProviderUrlPage.Add('GLM / Z.AI API URL:', False);
  ProviderUrlPage.Values[3] := 'https://api.z.ai/api/paas/v4';
  ProviderUrlPage.Add('OpenRouter API URL:', False);
  ProviderUrlPage.Values[4] := 'https://openrouter.ai/api/v1';

  DockerPage := CreateInputOptionPage(ProviderUrlPage.ID,
...
begin
  Result := False;
  if ((PageID = SecretPage.ID) or (PageID = ProviderUrlPage.ID)) and ExistingInstallerEnvAvailable() then
    Result := True;
end;
...
begin
  ...
  if ExistingInstallerEnvAvailable() then begin
  ...
  EnvText :=
    ...
    'OPENAI_API_KEY=' + EscapeEnvValue(SecretPage.Values[0]) + #13#10 +
    'OPENAI_API_BASE=' + EscapeEnvValue(ProviderUrlPage.Values[0]) + #13#10 +
    'MOONSHOT_API_KEY=' + EscapeEnvValue(SecretPage.Values[1]) + #13#10 +
    'MOONSHOT_API_BASE=' + EscapeEnvValue(ProviderUrlPage.Values[1]) + #13#10 +
    'DASHSCOPE_API_KEY=' + EscapeEnvValue(SecretPage.Values[2]) + #13#10 +
    'DASHSCOPE_API_BASE=' + EscapeEnvValue(ProviderUrlPage.Values[2]) + #13#10 +
    'ZAI_API_KEY=' + EscapeEnvValue(SecretPage.Values[3]) + #13#10 +
    'ZAI_API_BASE=' + EscapeEnvValue(ProviderUrlPage.Values[3]) + #13#10 +
    'OPENROUTER_API_KEY=' + EscapeEnvValue(SecretPage.Values[4]) + #13#10 +
    'OPENROUTER_API_BASE=' + EscapeEnvValue(ProviderUrlPage.Values[4]) + #13#10 +
    'TELEGRAM_BOT_TOKEN=' + EscapeEnvValue(SecretPage.Values[5]) + #13#10 +
    'DISCORD_BOT_TOKEN=' + EscapeEnvValue(SecretPage.Values[6]) + #13#10;
