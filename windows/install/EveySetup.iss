...
begin
  ...
  SecretPage := CreateInputQueryPage(PluginPage.ID,
    'API keys',
    'Enter at least one model provider key.',
    'OpenAI, Kimi, Qwen, GLM, and OpenRouter are supported. Values are hidden and written to .env. They are not passed to PowerShell command-line arguments.');
...
function HasModelProviderKey(): Boolean;
begin
  Result :=
    (Trim(SecretPage.Values[0]) <> '') or
    (Trim(SecretPage.Values[1]) <> '') or
    (Trim(SecretPage.Values[2]) <> '') or
    (Trim(SecretPage.Values[3]) <> '') or
    (Trim(SecretPage.Values[4]) <> '');
end;
...
begin
  ...
  if CurPageID = SecretPage.ID then begin
    if (not ExistingInstallerEnvAvailable()) and (not HasModelProviderKey()) then begin
      MsgBox('At least one AI provider API key is required.', mbError, MB_OK);
      Result := False;
    end;
  end;
...
begin
  ...
  if ExistingInstallerEnvAvailable() and (not HasModelProviderKey()) then begin
